...
int _main_(int /*_argc*/, char** /*_argv*/)
{
	...
	while (!entry::processEvents(viewState.m_width, viewState.m_height, debug, reset, &mouseState) )
	{
		...
		{
			lightPosRadius[ii][0] = sin( (lightTimeAccumulator*1.1f + ii*0.03f + float(ii*bx::piHalf)*1.07f ) )*20.0f;
			lightPosRadius[ii][1] = 8.0f + (1.0f - cos( (lightTimeAccumulator*1.5f + ii*0.29f + float(ii*bx::piHalf)*1.49f ) ))*4.0f;
			lightPosRadius[ii][2] = cos( (lightTimeAccumulator*1.3f + ii*0.13f + float(ii*bx::piHalf)*1.79f ) )*20.0f;
