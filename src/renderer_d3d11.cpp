		...
		struct RendererContextD3D11 : public RendererContextI
		{
			...
			bool init(const Init& _init)
			{
...

				{
					...
					{
						g_caps.supported |= BGFX_CAPS_COMPUTE;
						g_caps.supported |= BGFX_CAPS_STRUCTURED_BUFFERS;
					}
			...
			void shutdown()
			{
				...
				for (uint32_t ii = 0; ii < BX_COUNTOF(m_shaderBuffers); ++ii)
				{
					m_shaderBuffers[ii].destroy();
				}
			...
			{
				m_indexBuffers[_handle.idx].create(_mem->size, _mem->data, _flags, 0, BufferD3D11::USAGE_INDEX_BUFFER);
			}
			...
			{
				m_indexBuffers[_handle.idx].create(_size, NULL, _flags, 0, BufferD3D11::USAGE_INDEX_BUFFER);
			}
			...
			{
				m_shaderBuffers[_handle.idx].create(_size, NULL, 0, _stride, BufferD3D11::USAGE_SHADER_BUFFER);
			}

			void updateShaderBuffer(ShaderBufferHandle _handle, const Memory* _mem) override
			{
				m_shaderBuffers[_handle.idx].update(0, _mem->size, _mem->data, true);
			}
...
#undef CASE_IMPLEMENT_UNIFORM
			...
			BufferD3D11 m_shaderBuffers[BGFX_CONFIG_MAX_SHADER_BUFFERS];
		...
		void BufferD3D11::create(uint32_t _size, void* _data, uint16_t _flags, uint32_t _stride, Usage _usage)
		{
			bool isVertexBuffer = _usage == USAGE_VERTEX_BUFFER;
			bool isIndexBuffer = _usage == USAGE_INDEX_BUFFER;
			bool isShaderBuffer = _usage == USAGE_SHADER_BUFFER;
			...
			desc.ByteWidth = _size;
			desc.BindFlags = 0
				| (isVertexBuffer ? D3D11_BIND_VERTEX_BUFFER : (isIndexBuffer ? D3D11_BIND_INDEX_BUFFER : 0))
			...
			desc.MiscFlags = 0
				...
				| (isShaderBuffer ? D3D11_RESOURCE_MISC_BUFFER_STRUCTURED : 0)
				;
			desc.StructureByteStride = 0;
		...
		void VertexBufferD3D11::create(uint32_t _size, void* _data, VertexLayoutHandle _layoutHandle, uint16_t _flags)
		{
			...
			BufferD3D11::create(_size, _data, _flags, stride, BufferD3D11::USAGE_VERTEX_BUFFER);
		}
