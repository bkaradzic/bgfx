...
namespace bgfx { namespace d3d12
{
	...
	struct VideoDecoderD3D12
	{
		...
		bool decodeOneAU(const uint8_t* _bitstream, uint32_t _bitstreamSize, int64_t _ptsUs)
		{
			...
			HRESULT cres = m_copyCmdAllocator[copyRingIdx]->Reset();
			BX_WARN(SUCCEEDED(cres), "VideoDecoderD3D12: copy allocator Reset failed (0x%08x).", cres);

			cres = m_copyCmdList->Reset(m_copyCmdAllocator[copyRingIdx], NULL);
			BX_WARN(SUCCEEDED(cres), "VideoDecoderD3D12: copy cmdlist Reset failed (0x%08x).", cres);
			...
			cres = m_copyCmdList->Close();
			BX_WARN(SUCCEEDED(cres), "VideoDecoderD3D12: copy Close failed (0x%08x).", cres);
