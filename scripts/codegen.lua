...
function codegen.doxygen_type(doxygen, func, cname)
	...
	if cname then
		...
		if type(cname) == "string" then
			result[#result+1] = string.format("/// @attention The C99 bindings' equivalent is `%s`.", cname)
		else
			local names = {}
			for _, v in ipairs(cname) do
				names[#names+1] = "`" .. v .. "`"
			end
			result[#result+1] = string.format("/// @attention The C99 bindings' equivalents are %s.", table.concat(names, ","))
		end
	end
	result[#result+1] = "///"
